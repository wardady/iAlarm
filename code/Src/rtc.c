...
void set_alarm1(uint8_t sec, uint8_t min, uint8_t hour, uint8_t date)
{
	...
	aTxBuffer[8] = fromDEC(sec % 60) & 0b01111111;
	aTxBuffer[9] = fromDEC(min % 60) & 0b01111111;
	aTxBuffer[10] = fromDEC(hour % 24) & 0b01111111;
	aTxBuffer[11] = fromDEC(date % 31) & 0b01111111;
...
void set_alarm2(uint8_t min, uint8_t hour, uint8_t date)
{
	get_buffer();
...
void alarm2_timer(uint8_t min, uint8_t hour, uint8_t date)
{
	...
	aTxBuffer[14] = toDEC(aTxBuffer[4]) + (aTxBuffer[13] / 24) + date;

	aTxBuffer[12] = fromDEC(aTxBuffer[12] % 60) & 0b01111111;
	aTxBuffer[13] = fromDEC(aTxBuffer[13] % 24) & 0b01111111;
	aTxBuffer[14] = fromDEC(aTxBuffer[14] % 31) & 0b01111111;
